...
use warp_default_loader::Loader;
use warp_frontend::{
    analysis::{
    ...
    let loader = Loader::new(
        ...
        Some({
            warp_frontend::FilePath::Path(
                ...
                warp_frontend::helpers::InternedString::new(concat!(
                    env!("CARGO_WORKSPACE_DIR"),
                    "pkg/std/std.wpl"
                )),
                #[cfg(not(debug_assertions))]
                warp_frontend::helpers::InternedString::new(warp_default_loader::STD_URL),
            )
        }),
    );
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.borrow_client()
            .log_message(MessageType::INFO, "Warp language server initialized")
            .await;
    }
...
impl Backend {
    ...
    ) -> (Program, Vec<warp_frontend::diagnostics::Diagnostic>) {
