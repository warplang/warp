...
lazy_static! {
    static ref PLAYGROUND_PATH: warp_frontend::helpers::InternedString = warp_frontend::helpers::InternedString::new("playground");
    ...
    static ref LOADER: loader::Loader = {
        loader::Loader::new_with_fetcher(
            ...
            Some(warp_frontend::FilePath::Path(
                ...
                warp_frontend::helpers::InternedString::new(format!(
                    "{}pkg/std/std.wpl",
                    env!("CARGO_WORKSPACE_DIR")
                )),
                #[cfg(not(feature = "debug_playground"))]
                warp_frontend::helpers::InternedString::new(loader::STD_URL),
            )),
            Fetcher::new()
    ...
    static ref COMPILER: warp_frontend::Compiler<'static> = {
        let compiler = warp_frontend::Compiler::new(&*LOADER);
...
pub async fn analyze(id: String, code: String, lint: bool) -> JsValue {
    ...
    let (program, diagnostics) = COMPILER
        .analyze_with(
            warp_frontend::FilePath::Virtual(*PLAYGROUND_PATH),
            &warp_frontend::analysis::Options::new().lint(lint),
        )
        .await;
    ...
    let diagnostics = if diagnostics.is_empty() {
        ...
        match highest_level {
            Some(warp_frontend::diagnostics::DiagnosticLevel::Warning) => {
                AnalysisOutputDiagnostics::Warning(output)
            }
            Some(warp_frontend::diagnostics::DiagnosticLevel::Error) => {
...
fn get_syntax_highlighting(
    ...
    let playground_path = warp_frontend::FilePath::Virtual(*PLAYGROUND_PATH);
    ...
    insert_semantic_tokens!(
        ...
        |_, decl: &warp_frontend::analysis::typecheck::TemplateDecl| {
    ...
    let mut traverse_semantic_tokens = |expr: &warp_frontend::analysis::Expression| {
        ...
        if matches!(
            expr.kind,
            warp_frontend::analysis::ExpressionKind::Variable(_)
                | warp_frontend::analysis::ExpressionKind::Constant(_)
        ) && matches!(expr.ty, warp_frontend::analysis::Type::Function(_, _))
        {
            items.push(AnalysisOutputSyntaxHighlightingItem {
